...
"""
    ...
    # TODO corriger - utiliser regex !! -> 0-0 match avec 10-0
    def _filter_for_annotations(self, root, files, image_filename, v=False):
        ...
        files = [f for f in files if os.path.splitext(os.path.basename(f))[0].startswith(file_name_prefix)]
        # TODO CHANGE WITH STARTSWITH !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def clean_data(self, confirm=False):
        ...
        if confirm:
            ...
            for folder in [self._annot_coco_dir, 
    ...
    def split_image_annotations(self, v=False, force=False):
        """
        ...
        for _, _, files in os.walk(self._image_dir):
            ...
            for n, image_filename in enumerate(image_files):
                ...
                for _, _, annot in os.walk(self._instance_mask_dir):
                    ...
                    annotation_files = self._filter_for_annotations(self._instance_mask_dir, annot, image_filename, v=True) # PROBLEM HERE - Too Much Files
    ...
    def create_coco_annotations(self, v=False):
        ...
        logging.warning("Attention c'est long.")
        ...
        for _, _, files in os.walk(self._image_dir):
            ...
            for n, image_filename in enumerate(image_files):
                ...
                print(f"Processing image {n}/{total_images}")
